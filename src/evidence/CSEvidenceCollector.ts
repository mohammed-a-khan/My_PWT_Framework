...
export class CSEvidenceCollector {
    ...
    public async startCollection(testName: string): Promise<void> {
        ...
        // Start video recording if configured (support both old and new config keys)
        ...
        const videoEnabled = videoCaptureMode ?
            (videoCaptureMode !== 'never') :
            (browserVideo !== 'off');

        if (videoEnabled) {
            // Video recording is handled by browser context configuration
            CSReporter.debug(`Video recording started for: ${testName} (mode: ${videoCaptureMode || browserVideo})`);
        }
    ...
    private async captureHAR(): Promise<any> {
        try {
            ...
            // Check if HAR recording was enabled (support both old and new config keys)
            const harCaptureMode = this.config.get('HAR_CAPTURE_MODE');
            const harEnabled = this.config.getBoolean('BROWSER_HAR_ENABLED', false);

            // Support backward compatibility: HAR_CAPTURE_MODE takes precedence if set
            const harRecordingEnabled = harCaptureMode ?
                (harCaptureMode !== 'never') :
                harEnabled;
