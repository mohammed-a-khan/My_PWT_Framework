...
export class CSBDDRunner {
    ...
    private resolveFeaturePaths(features?: string | string[]): string[] {
        ...
        if (!features) {
            // Check for FEATURE_PATH environment variable (can contain multiple paths separated by ';')
            ...
            if (featurePath) {
                ...
                // Check for project-specific features directory
                ...
                // Default to test/features directory if nothing found
        ...
        // Resolve all paths and expand directories to feature files
    ...
    private async executeFeatures(features: ParsedFeature[], options: RunOptions): Promise<void> {
        ...
        const parallelEnabled = options.parallel === true || this.config.getBoolean('PARALLEL', false);
        const workers = options.workers || this.config.getNumber('WORKERS', 1);

        CSReporter.info(`Execution mode: parallel=${parallelEnabled}, workers=${workers}`);

        if (parallelEnabled && workers > 1) {
            CSReporter.info(`Starting parallel execution with ${workers} workers...`);
            await this.executeParallel(features, options, workers);
        } else {
            CSReporter.info(`Starting sequential execution...`);
    ...
    private async executeParallel(features: ParsedFeature[], options: RunOptions, workers: number): Promise<void> {
        // Check if we're already in a worker process
        if (process.env.PARALLEL_MODE === 'true' && process.env.WORKER_ID) {
            // We're in a worker, run sequentially
            await this.executeSequential(features, options);
            return;
        }
        ...
        try {
            ...
            results.forEach((result: any, workId: string) => {
                ...
                // Aggregate artifacts
                if (result.artifacts) {
                    if (result.artifacts.screenshots) allArtifacts.screenshots.push(...result.artifacts.screenshots);
                    if (result.artifacts.videos) allArtifacts.videos.push(...result.artifacts.videos);
                    if (result.artifacts.traces) allArtifacts.traces.push(...result.artifacts.traces);
                    if (result.artifacts.logs) allArtifacts.logs.push(...result.artifacts.logs);
                }
    ...
    private async executeSingleScenario(
        ...
        try {
            ...
            // Check if we should capture artifacts for passed scenarios
            ...
            // Capture artifacts for failed scenarios
            await this.captureArtifactsIfNeeded('failed');
            
            // Retry logic
            if (options.retry && options.retry > 0) {
                CSReporter.info(`Retrying scenario (attempt ${options.retry})...`);
                await this.executeSingleScenario(scenario, feature, { ...options, retry: options.retry - 1 }, exampleRow, exampleHeaders, iterationNumber, totalIterations, originalExamples);
    ...
    private async executeStep(
        ...
        // Declare preStepScreenshot at function scope so it's available in catch block
        let preStepScreenshot: string | undefined;
        ...
        try {
            ...
            // Capture pre-step screenshot for assertion steps (Then steps) in debug mode or when likely to fail
            const isAssertionStep = step.keyword.trim().toLowerCase() === 'then' ||
                                   stepText.toLowerCase().includes('should') ||
                                   stepText.toLowerCase().includes('verify') ||
                                   stepText.toLowerCase().includes('assert');

            if (isAssertionStep) {
                // Always try to capture pre-assertion screenshot for Then steps
                try {
                    const page = this.browserManager.getPage();
                    if (page && !page.isClosed()) {
                        ...
                        try {
                            ...
                            // Try to wait for any visible content (body should always be there)
                            await page.waitForSelector('body', { state: 'visible', timeout: 1000 }).catch(() => {});

                            // Small delay to ensure rendering is complete
                            await page.waitForTimeout(200);
                        } catch (waitError) {
                            CSReporter.debug(`Wait before screenshot had issues but continuing: ${waitError}`);
                        }
                        ...
                        preStepScreenshot = `${screenshotDir}/step-failure-${stepName}-${timestamp}.png`;

                        // Take screenshot after ensuring page stability
                        await page.screenshot({
                            path: preStepScreenshot,
                            fullPage: false
                        });

                        CSReporter.info(`Step screenshot captured: ${preStepScreenshot}`);

                        // Check file size to see if we got actual content
                        const stats = fs.statSync(preStepScreenshot);
                        CSReporter.info(`Step screenshot size: ${stats.size} bytes`);

                        // If screenshot is suspiciously small (likely blank), try again with different approach
                        if (stats.size < 10000) { // Less than 10KB is likely blank
                            CSReporter.warn(`Screenshot seems blank (${stats.size} bytes), attempting alternative capture`);

                            // Try waiting for network idle as last resort
                            ...
                            const newStats = fs.statSync(preStepScreenshot);
                            CSReporter.info(`Re-captured screenshot size: ${newStats.size} bytes`);
                        }
                    }
                } catch (err) {
                    CSReporter.warn(`Failed to capture pre-assertion screenshot: ${err}`);
                }
            }
            ...
            // Get actions from CSReporter for successful steps
            const reporterStep = CSReporter.getCurrentStep();
            const actions = reporterStep ? reporterStep.actions : [];

            this.scenarioContext.addStepResult(`${step.keyword} ${stepText}`, 'passed', duration, undefined, actions);
            ...
            // Clean up pre-step screenshot if assertion passed
            if (preStepScreenshot) {
                try {
                    const fs = require('fs');
                    if (fs.existsSync(preStepScreenshot)) {
                        fs.unlinkSync(preStepScreenshot);
                        CSReporter.debug('Deleted pre-assertion screenshot as step passed');
                    }
                } catch {
            ...
            // Get actions from CSReporter before adding step result
            const reporterStep = CSReporter.getCurrentStep();
            const actions = reporterStep ? reporterStep.actions : [];

            // Check if screenshot was already captured by CSExpect or CSAssert
            let screenshotFilename: string | undefined;
            const currentStep = this.scenarioContext.getCurrentStep();
            const existingScreenshots = this.scenarioContext.getScreenshots();

            // Check if a screenshot was already taken for this step (by CSExpect/CSAssert)
            const hasRecentScreenshot = existingScreenshots.length > 0 &&
                (Date.now() - existingScreenshots[existingScreenshots.length - 1].timestamp) < 2000; // Within 2 seconds

            // First check if we have a pre-step screenshot
            if (preStepScreenshot) {
                const path = require('path');
                screenshotFilename = path.basename(preStepScreenshot);
                if (currentStep) {
                    currentStep.screenshot = screenshotFilename;
                }
                this.scenarioContext.addScreenshot(preStepScreenshot, 'step-failure');
                CSReporter.info(`Using captured step screenshot: ${preStepScreenshot}`);
            } else if (currentStep && currentStep.screenshot) {
                // Screenshot was already captured and attached to current step
                screenshotFilename = currentStep.screenshot;
                CSReporter.debug('Using screenshot captured by assertion library');
            } else if (hasRecentScreenshot) {
                // A screenshot was recently captured (likely by CSExpect/CSAssert)
                const recentScreenshot = existingScreenshots[existingScreenshots.length - 1];
                const path = require('path');
                screenshotFilename = path.basename(recentScreenshot.path);
                if (currentStep) {
                    currentStep.screenshot = screenshotFilename;
                }
                CSReporter.debug('Using recently captured screenshot from assertion library');
            } else {
                // No screenshot was captured by assertion libraries, capture one now
                ...
                if (shouldCaptureScreenshot) {
                    try {
                        ...
                        if (page && !page.isClosed()) {
                            ...
                            const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
                            const stepName = `${step.keyword}-${stepText}`.replace(/[^a-zA-Z0-9]/g, '-').substring(0, 50);
                            const screenshotPath = `${screenshotDir}/step-failure-${stepName}-${timestamp}.png`;
                            await page.screenshot({
                                path: screenshotPath,
                            ...
                            // Attach screenshot to current step in scenario context
                            if (currentStep) {
                                currentStep.screenshot = screenshotFilename;
                            }
            ...
            // Now add step result with screenshot and actions
            this.scenarioContext.addStepResult(
                `${step.keyword} ${stepText}`,
                'failed',
                duration,
                screenshotFilename,
                actions
        ...
        try {
            ...
            const worldClassSuite = {
                ...
                scenarios: this.testSuite.features.flatMap(f => f.scenarios.map(s => ({
                    ...
                    feature: f.name,
                    tags: (s.tags || []).filter((tag: string) => !tag.startsWith('@data-config:')),
...
<body>
    ...
    private async captureStepScreenshot(stepName: string): Promise<void> {
        try {
            ...
            // Add to scenario context for reporting (if method exists)
            if (typeof (this.scenarioContext as any).getCurrentStep === 'function') {
                const currentStep = (this.scenarioContext as any).getCurrentStep();
                if (currentStep) {
                    const path = require('path');
                    currentStep.screenshot = path.basename(filepath);
                }
            }
