...
class CSChart {
    constructor(canvas, config) {
        ...
        this.legendArea = { x: 0, y: 0, width: 0, height: 0 };
    ...
    calculateAreas() {
        const padding = 20;
        const legendPosition = (this.options.plugins && this.options.plugins.legend && this.options.plugins.legend.position) || 'bottom';
        ...
        if (this.options.plugins && this.options.plugins.legend && this.options.plugins.legend.display !== false) {
            if (legendPosition === 'right') {
                ...
                const needsExtraSpace = this.type === 'bar' && this.options.scales && this.options.scales.x && this.options.scales.x.ticks && this.options.scales.x.ticks.maxRotation;
    ...
    startAnimation() {
        ...
        const duration = this.options.animation ? this.options.animation.duration || 1000 : 1000;
        ...
        const animate = () => {
            ...
            if (this.options.animation && this.options.animation.easing === 'easeOutQuart') {
    ...
    renderDoughnutChart() {
        ...
        dataset.data.forEach((value, index) => {
            ...
            if (this.animationProgress >= 1 && (!this.options.plugins || !this.options.plugins.datalabels || this.options.plugins.datalabels.display !== false)) {
                ...
                if (percentageValue >= 0.5) {
                    ...
                    this.ctx.fillStyle = (this.options.plugins && this.options.plugins.datalabels && this.options.plugins.datalabels.color) || '#fff';
                    const fontSize = (this.options.plugins && this.options.plugins.datalabels && this.options.plugins.datalabels.font && this.options.plugins.datalabels.font.size) || 20; // Much larger font
                    const fontWeight = (this.options.plugins && this.options.plugins.datalabels && this.options.plugins.datalabels.font && this.options.plugins.datalabels.font.weight) || 'bold';
    ...
    renderBarChart() {
        ...
        const isStacked = this.options.scales && this.options.scales.x && this.options.scales.x.stacked;
        ...
        for (let labelIndex = 0; labelIndex < barCount; labelIndex++) {
            ...
            if (this.options.scales && this.options.scales.x && this.options.scales.x.ticks && this.options.scales.x.ticks.maxRotation) {
    ...
    renderLineChart() {
        ...
        datasets.forEach((dataset, datasetIndex) => {
            const yAxisId = dataset.yAxisID || 'y';
            const maxValue = maxValues[yAxisId];
            ...
            dataset.data.forEach((value, index) => {
                ...
                points.push({ x, y, originalY: this.chartArea.y + this.chartArea.height - 60 - (value / maxValue) * (this.chartArea.height - 80) });
            });
        ...
        if (dataset.fill) {
            ...
            this.ctx.closePath();
            this.ctx.fillStyle = dataset.backgroundColor || 'rgba(59, 130, 246, 0.1)';
            this.ctx.fill();
        }
    ...
    renderLegend() {
        ...
        if (legendPosition === 'right') {
            ...
            items.forEach((label, index) => {
                ...
                this.ctx.fillStyle = Array.isArray(colors[index]) ? colors[index][0] : colors[index] || '#000';
    ...
    showTooltip(x, y, index) {
        ...
        if (this.options.plugins && this.options.plugins.tooltip && this.options.plugins.tooltip.callbacks && this.options.plugins.tooltip.callbacks.label) {
