...
export class CSWorldClassReportGenerator_Enhanced {
    ...
    public static generateReport(suite: TestSuite, outputDir: string): void {
        try {
            ...
            CSReporter.error(`Failed to generate world-class report: ${error}`);
            throw error;
        }
    }
    ...
    private static getEnvironmentInfo(suite?: TestSuite): any {
        ...
        return {
            ...
            test: {
                ...
                videoRecording: this.config.get('VIDEO_CAPTURE_MODE', 'on-failure') !== 'never' ? 'Enabled' : 'Disabled'
            },
            execution: {
                ...
                networkRecording: this.config.get('HAR_CAPTURE_MODE', 'always') === 'always' ? 'Enabled' : 'Disabled'
            },
            runtime: {
        ...
        function initializeDashboardCharts() {
            ...
            if (trendCtx && !charts.trend) {
                ...
                for (let i = 6; i >= 0; i--) {
                    ...
                    const historyEntry = ${JSON.stringify(history)}.find(h => h.date === dateStr);
                    trendValues.push(historyEntry ? historyEntry.passRate : 0);
                }
